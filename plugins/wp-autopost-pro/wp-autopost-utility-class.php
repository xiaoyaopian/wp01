        ...
        if ($autoSet) {
            ...
            for ($i = 0; $i < $matchNum; $i++) {
                if ($content_match_type[$i] == 0) {
                    switch ($objective[$i]) {
                        case '0':
                            $tempContent = gTheConUseCss($d, $content_selector[$i], $options, $charset, $outer[$i], $index[$i]);
                            break;
                    }
                } else {
                    switch ($objective[$i]) {
                        case '0':
                            $tempContent = getContentByRule($UTFhtml, $content_selector[$i], $options, $outer[$i]);
                            break;
                    }
                }
            }
...
class autopostBaiduTranslator
{
    ...
    public static function translate($src_text, $fromLanguage, $toLanguage, $appId='', $appSecret='', $GET = false)
    {
        $BaiduTransOptions = get_option('wp-autopost-baidu-trans-options');
        $appId = $BaiduTransOptions['app_id'];
        $appSecret = $BaiduTransOptions['app_secret'];
        ...
        try {
            ...
            $params = array(
                'q' => $src_text,
                'from' => urlencode($fromLanguage),
                'to' => urlencode($toLanguage),
                'appid' => $appId,
                'salt' => time(),
            );
            $params['sign'] = md5($params['appid'] . $params['q'] . $params['salt'] . $appSecret);
            if ($GET) {
                $translateUrl .= '?' . http_build_query($params);
				$curlResponse = self::curlRequest($translateUrl);
			} else {

                $curlResponse = self::curlRequest($translateUrl, $params);
            }
            ...
            if (isset($re->error_code)) {
                ...
                switch ($re->error_code) {
                    case '52001':
                        $translated['err'] .= '[Time Out]';
                        break;
                    case '52002':
                        $translated['err'] .= '[The translator system error, try later]';
                        break;
                    case '52003':
                        $translated['err'] .= '[Unauthorized, please check your APP ID and APP SECRET]';
                        break;
                }
            } else {
    ...
    protected function parse_tag_attr($tag, $name, $i)
    {
        ...
        switch ($name) {
            ...
            case '\'':
                ...
                break;
            default:
                $attrname = parse_tag_attr_name($tag);
                break;
        }
        return $attrname;
    }
