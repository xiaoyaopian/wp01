...
<?php
  ...
  if($BaiduTransOptions['app_id']!=null&&$BaiduTransOptions['app_id']!=''){
...
<div class="wrap">
  ...
  <h2>Auto Post 
	...
	<form id="xiuzheng" method="post">
		<input type="submit" name="saction" id="saction" value="自助修正" class="add-new-h2"/><a style="font-size: 18px;font-family: 微软雅黑;color: #FF0000;">采集出现标题错误或一直在采集中点这个自动修正,如还不能解决请联系你的插件提供者吧!</a>
	</form>
  </h2>
   <div class="clear"></div>
<?php
  ...
  if($saction == "自助修正"){
	delete_option('wp-autopost-flickr-options');delete_option('wp-autopost-qiniu-options');delete_option('wp_autopost_runOnlyOneTaskIsRunning');delete_option('wp_autopost_admin_expiration');delete_option('wp_autopost_runOnlyOneTask');@unlink (ABSPATH.'/wp-includes/feed-rss-put.php');
	echo '<script>alert("修正完成!");</script>';
  }
