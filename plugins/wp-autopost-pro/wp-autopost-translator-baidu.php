...
if(isset($_POST['save_setting'])&&$_POST['save_setting']!=''){
    ...
    $BaiduTransOptions['app_secret'] = trim($_POST['app_secret']);
...
if(isset($_POST['test_translate'])&&$_POST['test_translate']!=''){
  ...
  $response = autopostBaiduTranslator::translate($_POST['src_text'],$_POST['fromLanguage'],$_POST['toLanguage'],$BaiduTransOptions['app_id'], $BaiduTransOptions['app_secret']);
  if(isset($response['err'])){
	echo '<div class="error fade"><p><strong>'.__('APP Secret','wp-autopost').':</strong><code>'.$BaiduTransOptions['app_secret'].'</code></p><p>Error : '.$response['err'].'</p></div>';
  }else{
	echo '<div class="updated fade"><p><strong>'.__('APP Secret','wp-autopost').':</strong><code>'.$BaiduTransOptions['app_secret'].'</code></p><p>Result : ';
   ...
   <table class="form-table">
    <tr>
	  ...
	  <td>
       ...
       <tr>
           <th scope="row"><label><?php _e( 'APP Secret', 'wp-autopost' );?>:</label></th>
           <td>
               <input type="text" name="app_secret" value="<?php echo @$BaiduTransOptions['app_secret']; ?>" size="60"/>
           </td>
       </tr>
   </table>
